...
12. Binding (привязка) //TODO

    Добавим в разметку слайдер, задав ему название, минимальное и максимальное значения:

    ```xml
    <Slider Minimum="10" Maximum="100" Name="MySlider"/>
    ```

    И в заголовке окна вместо MainWindow напишем следующее:

    ```xml
    Title="{Binding Value, ElementName=MySlider}"
    ```

    Вместо фиксированного значения мы вставляем выражение, где *Binding* означает привязку к аттрибуту *Value* у эдемента с названием *MySlider*. 
    
    Таким образом мы можем привязать любой аттрибут (в скринкасте слайдером меняют размер шрифта у кнопки).

    Но привязка к элементам это игрушки, основная задача этой технологи - привязка к переменным в коде приложения.

    В коде окна добавим переменную:
