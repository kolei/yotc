...
- проверять сортировку по обоим условиям
- проверить работу при разных размерах исходного списка 
- проверить работу при пустом списке
- проверить работу при дате вне диапазона (`new DateTime()` без параметров вернет текущую дату, по условию даты должны быть за предыдущий год, если есть кривая дата, то либо выбрасывать исключение, либо не включать в список (лучше второй вариант - про то что библиотека не должна выбрасывать исключений вроде нигде не написано))
